  ...
  const columns = [
    {
      ...
      selector: (row) => row[0]?.location[0]?.name,
      cell: (row) => (
        <div className="d-flex justify-content-left align-items-center">
          {row[0]?.location[0]?.name}
        </div>
      )
    },
    {
      name: 'Address',
      center: true,
      selector: (row) => row[0]?.location[0]?.address?.city,
      cell: (row) => (
        <div className="d-flex justify-content-left align-items-center">
          {row[0]?.location[0]?.address?.city}
        </div>
      )
    },
    {
      name: 'Athlete',
      selector: (row) => row?.filter((item) => item.registrantType === 'Athlete')?.length,
      cell: (row) => (
        <span className="d-flex justify-content-left align-items-center">
          {row?.filter((item) => item.registrantType === 'Athlete')?.length}
        </span>
      )
    },
    {
      name: 'Coach',
      selector: (row) => row?.filter((item) => item.registrantType === 'Coach')?.length,
      cell: (row) => (
        <div className="d-flex justify-content-center align-items-center">
          {row?.filter((item) => item.registrantType === 'Coach')?.length}
        </div>
      )
    },
    {
      name: 'Referee',
      selector: (row) => row?.filter((item) => item.registrantType === 'Referee')?.length,
      cell: (row) => (
        <div className="d-flex justify-content-left align-items-center">
          {row?.filter((item) => item.registrantType === 'Referee')?.length}
        </div>
      )
    },
    {
