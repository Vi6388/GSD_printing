...
import { resetUserPasswordAction } from '../account/store/actions';
import { useDispatch } from 'react-redux';
import { toast } from "react-toastify";
import { changePasswordAdminInfoData } from '../../../../requests/admin/AdminAPI';
...
const Security = () => {
  ...
  const onSubmit = async (data) => {
    console.log(data);
    const response = await changePasswordAdminInfoData(data);
    if(response?.status) {
      toast.success(
        <div className="toastify-header">
           <div className="title-wrapper">
           <h6 className="toast-title fw-bold">Password changed Successfully.</h6>
           </div>
        </div>
     );
    } else {
