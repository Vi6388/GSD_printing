    ...
    <Card className="shadow-none border cursor-pointer">
      ...
      <CardBody>
        ...
        <div className="d-flex justify-content-between mb-50">
          <span className="text-truncate">{props.item.usedStore}GB Used</span>
          <small className="text-muted">{props.item.totalStore}GB</small>
        </div>
        {props.item.id == 1 && (
          <Progress
            className="progress-bar-danger"
            value={(props.item.usedStore / props.item.totalStore) * 100}
          />
        )}
        {props.item.id == 2 && (
          <Progress
