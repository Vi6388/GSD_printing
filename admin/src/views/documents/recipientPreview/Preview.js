...
export default function Preview() {
  ...
  useEffect(() => {
    ...
    const panzoomInstance = panzoom(mainContainer, {
      ...
      initialZoom: isMobile ? 0.4 : 1,
      bounds: true,
      boundsPadding: 0.2
    });

    isMobile ? setZoom(0.4) : setZoom(1);
    setZoomInstance(panzoomInstance);
  }, []);
  ...
  useEffect(() => {
    if (zoomInstance) {
      zoomInstance.on('transform', function (e) {
        if (isMobile) {
          if (
            e.getTransform().scale <= 0.4 &&
            e.getTransform().x !== 0 &&
            e.getTransform().y !== 0
          ) {
            e.moveTo(0, 0);
          } else {
            e.moveTo(e.getTransform().x, 0);
          }
        } else {
          if (
            e.getTransform().scale <= 1.1 &&
            e.getTransform().x !== 0 &&
            e.getTransform().y !== 0
          ) {
            e.moveTo(0, 0);
        ...
        <>
          ...
          <div>
            <div
              ...
              style={{
                width: `${isMobile ? window.innerWidth : boardDimention.x + 20}px`,
                overflow: 'hidden'
              }}
              id="boundDiv"
            >
