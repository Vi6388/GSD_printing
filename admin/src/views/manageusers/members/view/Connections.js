...
const connections = ({ contact }) => {
  ...
  const [socialLinks, setSocialLinks] = useState(
    singleClient?.client?.socialLinks ? singleClient?.client?.socialLinks : []
  );
  ...
  useEffect(() => {
    contact?.socialLinks?.map((l) => {
      ...
      setSocialLinks(singleClient?.client?.socialLinks ? singleClient?.client?.socialLinks : []);
    });
  }, []);
  ...
  useMemo(() => {
    if (socialLink.isSuccess) {
      // Update Message
      toast.success(<ToastContent message="Social Link Updated successfull" />);
