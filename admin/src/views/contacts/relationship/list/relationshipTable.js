...
import {
  useGetRelationContacts,
  useGetRelationPosition
} from '../../../../requests/contacts/relationship-contacts';
...
import { ChevronDown, Share, Printer, FileText, File, Grid, Copy, Upload } from 'react-feather';
      ...
      >
        ...
        <ModalFooter>
          ...
          <Button
            ...
            onClick={() => {
              setCurrentPage(1);
              dispatch(deleteContactAction({ _id: deleteModal?.id }));
            }}
          >
            ...
            onClick={() => {
              if (contactImportStep === 'first') {
                ...
                const CheckInvalidType = contacts.find((x, i) => {
                  if (x[3] === 'personal' || x[3] === 'business' || x[3] === 'other') {
                    return false;
                  }
                  return true;
                });
                ...
                if (CheckInvalidType) {
                  error('Type Column must have value individual or company');
                  return;
                }
                ...
                if (CheckInvalidPosition) {
                  error('Position Column must have value owner / assitant / billing / n/a');
                  return;
                }
