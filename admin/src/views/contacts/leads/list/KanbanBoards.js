...
const KanbanBoards = (props) => {
  ...
  const [boardDataHot, setBoardDataHot] = useState(
    store.data == null ? [] : store.data.list.filter((data) => data.stage === 'hot')
  );
  const [boardDataWarm, setBoardDataWarm] = useState(
    store.data == null ? [] : store.data.list.filter((data) => data.stage === 'warm')
  );
  const [boardDataCold, setBoardDataCold] = useState(
    store.data == null ? [] : store.data.list.filter((data) => data.stage === 'cold')
  );
