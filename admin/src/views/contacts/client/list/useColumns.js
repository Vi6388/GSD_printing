...
const useColumns = ({ setDeleteModal }, { toggle }) => {
  ...
  const columns = [
...

    {
      ...
      cell: (row) => (
        <span className="text-capitalize">{row.companyAddress}</span>
      ),
    },
    {
      name: "Phone",
    ...
    },
    {
      ...
      width: "150px",
      selector: (row) => row.phoneSecondary,
      cell: (row) => <span>{row.phoneSecondary}</span>,
    },
    {
      name: "Tags",
      width: "150px",
    ...
    //   cell: (row) => <Note toggle={toggle} note={row?.note} id={row._id} />
    //   // <Eye />
    // }
    // {
    //   name: "Tag",
