...
import { addRankReset, editRankReset, deleteRankReset } from '../../store/reducer';
...
const data = [
  {
    ...
    imgSrc: require('@src/assets/images/icons/certifications/nasm.png').default
  }
];
...
const Certifications = ({ selectedUser }) => {
  ...
  function handleAddRank() {
    ...
    if (rankName === '') {
      toast.error(<ToastContent message="Rank name must not be empty !" />);
    } else if (photo === '') {
    ...
    <Fragment>
                  ...
                  >
                    <div className="d-flex justify-content-between flex-sm-row flex-column">
                      ...
                      <div className="d-flex flex-column text-start text-lg-end">
                        ...
                        <span className="mt-2">
                          Got Certificate at {useDateFormatter(card.createdAt)}
                        </span>
                      </div>
                    </div>
                  </div>
      ...
      >
        ...
        <ModalBody className="px-sm-5 mx-50 pb-5">
          <h1 className="text-center mb-1">{selectedCondition ? 'Edit' : 'Add New'} Certificate</h1>
          ...
          <Row tag={Form} className="gy-1 gx-2 mt-75" onSubmit={onSubmit}>
            ...
            <Col className="text-center mt-1" xs={12}>
              <Button
                onClick={selectedCondition ? handleEdit : handleAddRank}
                type="button"
                className="mt-1 me-1"
                color="primary"
                disabled={value?.rank?.isLoading || editLoading}
              >
                {value?.rank?.isLoading || editLoading ? 'Processing...' : 'Save'}
              </Button>
