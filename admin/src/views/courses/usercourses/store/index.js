...
export const createVideo = createAsyncThunk(
  ...
  async (eventForm, { dispatch }) => {
    const response = await API.post(`video/create`, videoForm);
    await dispatch(getVideos(getUserData().id));
    return response.data;
  }
);

export const getVideos = createAsyncThunk('video/getVideos', async (userId) => {
  const response = await API.get(`video/all/${userId}`);
  return response.data;
});

export const getVideoInfo = createAsyncThunk('video/getVideoInfo', async (videoId) => {
